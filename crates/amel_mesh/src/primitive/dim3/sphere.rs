...
impl SphereMeshBuilder {
    ...
    pub fn build(&self) -> Mesh {
        match self.kind {
            SphereKind::Ico { subdivisions } => match self.ico(subdivisions) {
                Ok(mesh) => mesh,
                Err(IcosphereError::TooManyVertices {
                    subdivisions,
                    number_of_resulting_points,
                }) => {
                    panic!(
                        "Cannot create an icosphere of {} subdivisions due to there being too many vertices being generated: {}.",
    ...
    pub fn ico(&self, subdivisions: usize) -> Result<Mesh, IcosphereError> {
        ...
        let indices: Vec<u32> = if self.wireframe {
            ...
            for i in 0..20 {
                generated.get_line_indices(&mut wireframe_indices, i, 0, |buffer| {
                    // Add a break in the line strip (you can modify this as necessary)
                    buffer.push(u32::MAX);
                });
            }
            wireframe_indices
        } else {
            let mut solid_indices = Vec::with_capacity(generated.indices_per_main_triangle() * 20);
            for i in 0..20 {
                generated.get_indices(i, &mut solid_indices);
            }
            solid_indices
        };
        ...
        Ok(Mesh::new(primitive_topology)
