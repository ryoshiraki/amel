...
impl Context {
    async fn init_async<A: App>(surface: &mut SurfaceWrapper, window: Arc<Window>) -> Self {
...
async fn start<A: App>(title: &str) {
    log::debug!(
        "Enabled backends: {:?}",
    ...
    let context = Context::init_async::<A>(&mut surface, window_loop.window.clone()).await;
...
pub fn run<A: App>(title: &'static str) {
    cfg_if::cfg_if! {
        if #[cfg(target_arch = "wasm32")] {
            wasm_bindgen_futures::spawn_local(async move { start::<E>(title).await })
        } else {
            pollster::block_on(start::<A>(title));
        }
    }
}